...
- `data/supplements.json` — 26 supplements with `upperLimit` (text), `rda`, `timing`, a `sourceHint` pointing to the relevant NIH Office of Dietary Supplements fact sheet, plus structured `unit` (string | null) and `upperLimitValue` (number | null) fields used for numeric dose checking (see below).
